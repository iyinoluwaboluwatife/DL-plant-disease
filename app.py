...
PATH = r'C:\Users\IYINOLUWA\Desktop\Deep-Learning-plant_disease_identification\model'
...
def all_upload():
    ...
    if request.method == 'POST':
        try:
            ...
            queries = []
            for what in uploads:
                if query.lower().strip()[:-3] in what.plant:
                    queries.append(what)
...
def upload():
    ...
    if request.method == 'POST':
        ...
        result, plant_name, label, scale = model_predict(file_path, model, plant_name, delete)
        print(result, plant_name, label, scale)
